...
class MX():
    def __init__(self, id: str, message: discord.Message, channelID: int = 1067638742082400318, initatorID: int = 740632983630905445, download_speed: int = 5, upload_speed: int = 0.5):
        self.id = id
        ...
        for page in pages:
            with requests.get(page, stream=True, headers=self.header) as response:
                if response.status_code == 200:
                    ...
                    file.seek(0)
                    media.append(discord.File(file, filename=page))
